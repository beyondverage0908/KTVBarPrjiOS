    ...
    [KTVMainService postMainPage:self.mainParams result:^(NSDictionary *result) {
        if ([result[@"code"] isEqualToString:ktvCode] && [result[@"data"] count]) {
            for (NSDictionary *dict in result[@"data"]) {
                ...
                NSLog(@"-->> %@ -->> %@", storeContainer.store.storeId, storeContainer.store.storeName);
            }
    ...
    if (section == 0) {
        ...
        filterView.filterCallback = ^(NSInteger index, NSDictionary *filterMap) {
            ...
            if (@available(iOS 11.0, *)) {
                [weakself.tableView setContentOffset:CGPointMake(0, -tableView.adjustedContentInset.top + 145.0f) animated:NO];
            } else {
                [weakself.tableView setContentOffset:CGPointMake(0, -tableView.contentInset.top + 145.0f) animated:NO];
            }
        };
