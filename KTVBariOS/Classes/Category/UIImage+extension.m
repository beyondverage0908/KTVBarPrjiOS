...
//  GongYunTong
//
//  Created by pingjun lin on 2017/5/22.
...
- (UIImage *)resetSizeOfImageData:(UIImage *)source_image maxSize:(NSInteger)maxSize {
    ...
    UIImage *finallImage = [UIImage imageWithData:finallImageData];
    return finallImage;
}
...
- (NSData *)halfFuntion:(NSArray *)arr image:(UIImage *)image sourceData:(NSData *)finallImageData maxSize:(NSInteger)maxSize {
    ...
    while(start <= end) {
        ...
        NSUInteger sizeOrigin = finallImageData.length;
        NSUInteger sizeOriginKB = sizeOrigin / 1024;
        CLog(@"当前降到的质量：%ld", (unsigned long)sizeOriginKB);
        CLog(@"%lu----%lf", (unsigned long)index, [arr[index] floatValue]);
